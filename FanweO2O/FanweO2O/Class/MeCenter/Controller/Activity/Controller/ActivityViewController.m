...
@interface ActivityViewController ()<UITableViewDelegate,UITableViewDataSource,SecondaryNavigationBarViewDelegate,PopViewDelegate,QrCodeViewDelegate>
{
...
- (void)updateNewData {
    ...
    [dic setObject:@"uc_event" forKey:@"ctl"];
...
//        HideIndicatorInView(self.view);
        if ([responseJson toInt:@"status"] ==1) {
            ...
            _dateArray=[ActivityModel mj_objectArrayWithKeyValuesArray:responseJson[@"item"]];
            [self.tableView.mj_header endRefreshing];
            
            [_tableView tableViewDisplayWitMsg:@"暂无活动券" ifNecessaryForRowCount:_dateArray.count];
...
- (void)updateMoreData {
    ...
    [self.httpManager POSTWithParameters:dic SuccessBlock:^(NSDictionary *responseJson) {
        if ([responseJson toInt:@"status"] ==1) {
            ...
            NSArray *array =[ActivityModel mj_objectArrayWithKeyValuesArray:responseJson[@"item"]];
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    ActivityModel *cModel=_dateArray[section];
    vc.aModel =cModel;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ActivityModel *cModel =_dateArray[indexPath.section];
    if ([cModel.status intValue] ==1) {
        qrView.qrCodeNumber.text =[NSString stringWithFormat:@"劵码: %@",cModel.sn];
...
{
    NSString *str = [NSString stringWithFormat:@"%ld",tap.view.tag - 3000];
    ActivityModel *cModel=_dateArray[[str intValue]];
    FWO2OJumpModel *jumpModel =[FWO2OJumpModel new];
    NSString *urlString =[NSString stringWithFormat:@"%@?ctl=event&data_id=%@",
                          API_LOTTERYOUT_URL,cModel.event_id];
