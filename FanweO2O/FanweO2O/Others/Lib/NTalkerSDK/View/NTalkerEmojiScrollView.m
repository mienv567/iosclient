...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat gapH = 20;
        CGFloat gapW = (kFWFullScreenWidth-6*faceIconW)/7;
        ...
        for (int i=0; i<20; i++) {
            ...
            int line = i/6;
            int location = i%6;
            CGFloat faceIconX = location*(faceIconW+gapW) + gapW;
        ...
        [sendBtn setFrame:CGRectMake(kFWFullScreenWidth-gapW-sendBtnW, 3*(faceIconH+gapH)+18, sendBtnW, sendBtnH)];
        [sendBtn setTitle:NSLocalizedStringFromTable(@"send", @"XNLocalizable", nil) forState:UIControlStateNormal];
        ...
        deleteBtn.tag = 21;
        [deleteBtn setFrame:CGRectMake(2*(faceIconW+gapW)+gapW,3*(faceIconH+gapH)+gapH , faceIconW, faceIconH)];
        [deleteBtn setBackgroundImage:[UIImage imageNamed:@"NTalkerUIKitResource.bundle/ntalker_emoji_cancel.png"] forState:UIControlStateNormal];
