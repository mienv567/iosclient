...
- (void)setChatImageMessageInfo:(XNBaseMessage *)dto  hidden:(BOOL)hide{
    ...
    if (dto.msgType == 2) {
        ...
        if (dto.usericon && ![dto.usericon isEqualToString:@""]) {
            ...
            [_headIcon NTalkersd_setImageWithURL:[NSURL URLWithString:headImageUrl] placeholderImage:[UIImage imageNamed:@"NTalkerUIKitResource.bundle/ntalker_KF_icon.png"]];
        }
//修复大表情显示bug
...
- (void)handleContentImage:(XNBaseMessage *)dto{
    ...
    if (imageMessage.isEmotion) {
        // 表情大小
        CGFloat faceIconW = 18 * autoSizeScaleX;
        CGFloat faceIconH = 18 * autoSizeScaleX;
        UIImage *contentBgImage = [UIImage imageNamed:@"NTalkerUIKitResource.bundle/ntalker_chat_left.png"];
        [contentBg setImage:[contentBgImage stretchableImageWithLeftCapWidth:18 topCapHeight:38]];//30 20  易到的
        [self insertSubview:contentImage aboveSubview:contentBg];
        ...
        if ([fileManager fileExistsAtPath:path]) {
            ...
            newImageSize = CGSizeMake(faceIconW, faceIconH);
            CGRect frame= contentBg.frame;
            
            contentBg.frame = CGRectMake(64, frame.origin.y, newImageSize.width*autoSizeScaleX + 24*autoSizeScaleX, newImageSize.height*autoSizeScaleX + 20 *autoSizeScaleY);
            
            contentImage.frame = CGRectMake(64 + 12 * autoSizeScaleX, frame.origin.y + 10*autoSizeScaleY, newImageSize.width*autoSizeScaleX, newImageSize.height*autoSizeScaleX);
            
            contentBtn.frame= CGRectMake(64, frame.origin.y, newImageSize.width*autoSizeScaleX + 24*autoSizeScaleX, newImageSize.height*autoSizeScaleX + 20 *autoSizeScaleY);
            
            [self setFrame:CGRectMake(0, 0, kFWFullScreenWidth,frame.origin.y+(contentBtn.frame.size.height+1)*autoSizeScaleY)];
        ...
        UIImage *contentBgImage = [UIImage imageNamed:@"NTalkerUIKitResource.bundle/ntalker_image_chat_left.png"];
        [contentBg setImage:[contentBgImage stretchableImageWithLeftCapWidth:18 topCapHeight:38]];//30 20  易到的
        [self insertSubview:contentBg aboveSubview:contentImage];
