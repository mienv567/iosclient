...
- (void)setChatImageMessageInfo:(XNBaseMessage *)dto  hidden:(BOOL)hide{
    ...
    if (dto.msgType == 2) {
        ...
        [contentBg setImage:[contentBgImage stretchableImageWithLeftCapWidth:18 topCapHeight:38]];
        [contentImage setImage:[contentBgImage stretchableImageWithLeftCapWidth:18 topCapHeight:38]];
    ...
    if ([fileManager fileExistsAtPath:path]) {
        ...
        if (imageSize.height!=imageSize.width) {
            ...
            if (dto.sendStatus == SS_SENDING || dto.sendStatus == SS_TOSEND) {
                //test
                CGFloat failedBtnY = CGRectGetMinY(contentBtn.frame)+((CGRectGetHeight(contentBtn.frame)-25)/2);
                if (CGRectGetHeight(contentBtn.frame)<25) {
                    failedBtnY = CGRectGetMinY(contentBtn.frame)-(25-CGRectGetHeight(contentBtn.frame)/2);
                }
                [indicatorView setFrame:CGRectMake(CGRectGetMinX(contentImage.frame) - 25*autoSizeScaleX, failedBtnY, 25*autoSizeScaleX, 25*autoSizeScaleX)];
                ...
                [failedBtn setFrame:CGRectMake(CGRectGetMinX(contentImage.frame) - 25, failedBtnY, 25, 25)];
                failedBtn.hidden=NO;
            ...
            if (dto.sendStatus == SS_SENDING || dto.sendStatus == SS_TOSEND) {
                ...
                [failedBtn setFrame:CGRectMake(CGRectGetMinX(contentImage.frame) - 25 - 3, failedBtnY, 25, 25)];
                failedBtn.hidden=NO;
                [self.sendFailedArray addObject:dto];
        ...
        if (![imageMessage.pictureSource isEqualToString:@""] && imageMessage.pictureSource!=nil) {
            NSError *error;
            //默认取大图
            NSString *paths =[imageMessage.pictureSource stringByReplacingOccurrencesOfString:@"&amp;" withString:@"&"];
            //如果有小图 用小图的
            if (![imageMessage.pictureThumb isEqualToString:@""] && imageMessage.pictureThumb!=nil) {
                paths =[imageMessage.pictureThumb stringByReplacingOccurrencesOfString:@"&amp;" withString:@"&"];
            }
            //0420 崩溃bug
            if (!imageMessage.pictureName) {
                return;
            }
...
//            } else if(imageData){
                ...
                [contentImage NTalkersd_setImageWithURL:[NSURL URLWithString:imageMessage.pictureSource] placeholderImage:nil];
                CGSize imageSize = image.size;
                if (imageSize.height!=imageSize.width) {
                    CGSize newImageSize;
                    if (imageSize.height>imageSize.width) {
