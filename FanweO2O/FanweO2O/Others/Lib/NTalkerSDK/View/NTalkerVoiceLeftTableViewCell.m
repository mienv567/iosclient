...
- (void)setChatVoiceMessageCell:(XNBaseMessage *)dto  hidden:(BOOL)hide{
    ...
    }else if (voiceMessage.voiceLength>60){
        //超过60秒按60秒长度显示
        length = 75+(60-5)*2;
    }else {
    ...
    [tapControl setBackgroundImage:[contentBgImage stretchableImageWithLeftCapWidth:22 topCapHeight:16] forState:UIControlStateNormal];
    [tapControl setBackgroundImage:[contentBgImageS stretchableImageWithLeftCapWidth:22 topCapHeight:16] forState:UIControlStateHighlighted];
    ...
    if (voiceMessage.voiceLength>60) {
        ...
        longLabel.text = [NSString stringWithFormat:@"%ld''",voiceMessage.voiceLength];
    }
    longLabel.hidden = NO;
    [longLabel sizeToFit];
    CGRect longFrame = longLabel.frame;
    //左边语音时间距离远bug
    longFrame.origin.x = CGRectGetMaxX(tapControl.frame)+3;
