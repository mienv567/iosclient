...
- (void)setChatVoiceMessageCell:(XNBaseMessage *)dto  hidden:(BOOL)hide{
    ...
    if (dto.msgType == 6) {
        ...
        [contentImage setImage:[UIImage imageNamed:@"NTalkerUIKitResource.bundle/ntalker_sound0.png"]];
        [tapControl setBackgroundImage:[contentBgImage stretchableImageWithLeftCapWidth:17 topCapHeight:33] forState:UIControlStateNormal];
        [tapControl setBackgroundImage:[contentBgImageS stretchableImageWithLeftCapWidth:17 topCapHeight:33] forState:UIControlStateHighlighted];
        ...
        if (dto.sendStatus == SS_SENDSUCCESS || dto.sendStatus == SS_RECEIVESUCCESS) {
            ...
            longLabel.hidden=NO;
            //60+
            if (voiceMessage.voiceLength>60) {
                longLabel.text = @"60''+";
            }else{
                longLabel.text = [NSString stringWithFormat:@"%ld''",(long)voiceMessage.voiceLength];
            }
            [longLabel sizeToFit];
