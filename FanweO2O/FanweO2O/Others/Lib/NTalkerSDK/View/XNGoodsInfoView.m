...
- (void)configureGoodsIdView
{
    ...
    if (_goodsInfo.goods_id.length) {
        NSString *URLString = [NSString stringWithFormat:@"%@//goodsinfo/api.php?siteid=%@&itemid=%@&itemparam=%@&sellerid=&user_id=%@&type=json&from=IOSSDK&version=%@",basicInfo.serverData.manageserver?:@"",basicInfo.siteid,_goodsInfo.goods_id,_goodsInfo.itemparam?:@"",basicInfo.shortUid?:@"",basicInfo.version?:@""];//itemparam bug
        [[[XNHttpManager sharedManager] getFirstHttpService] sendProductInfoWithURL:URLString andParam:nil andBlock:^(id response){
            if ([response isKindOfClass:[NSDictionary class]]) {
                ...
                if ([obj isKindOfClass:[NSDictionary class]]) {
                    ...
                    _goodsInfo.goodsPrice = ((NSString *)dict[@"siteprice"]).length?dict[@"siteprice"]:dict[@"marketprice"];
...
- (void)addSubviewsInGoodsView
{
    ...
    if (_goodsInfo.goods_imageURL.length) {
        [imageView NTalkersd_setImageWithURL:[NSURL URLWithString:_goodsInfo.goods_imageURL] placeholderImage:nil];
    }
