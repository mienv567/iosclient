...
- (void)initUI
{
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        if ([path hasPrefix:@"http:"]||[path hasPrefix:@"https:"]) {
            image = [[NTalkerSDImageCache sharedImageCache] imageFromDiskCacheForKey:path];
            ...
            if (!image) {
                //第一次显示小图的 bug
                [self showActivityViewWithFrame:CGRectMake((KSCREENSIZE.width - 25)/2, 300, 25, 25) andTag:1020 inView:zoomScrollView];
                
                [[NTalkerSDWebImageManager sharedManager] downloadImageWithURL:URLPath
                                                              ...
                                                              completed:^(UIImage *image, NSError *error, NTalkerSDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                                                  if (finished) {
                                                                      //首次小图 bug
                                                                      [self hideActivityViewWithTag:1020 inView:zoomScrollView];
                                                                      imageView.image = image;
                                                         //
                                                                      if (image.size.width >=KSCREENSIZE.width) {//test
...
//    PZLWJSBridgeImageData *data = _imageFrames.imageList[_imageIndex];
    ...
    if ([currentPath hasPrefix:@"http:"]||[currentPath hasPrefix:@"https:"]) {
        currentImage = [[NTalkerSDImageCache sharedImageCache] imageFromDiskCacheForKey:currentPath];
        if (!currentImage) {
            ...
            if ([obj isKindOfClass:[UIScrollView class]]) {
               UIScrollView *scrollView = self.subviews[0];
                ...
                if (imageView.image) {
                   ...
                   [self showToastWithContent:NSLocalizedStringFromTable(@"successSaveToLibrary", @"XNLocalizable", nil) andRect:CGRectMake((KSCREENSIZE.width - 200)/2, (KSCREENSIZE.height - 90 - 20)/2, 200, 50) andTime:1 inView:self];
                }
            }
           break;
        }
        default:{
            [self showToastWithContent:NSLocalizedStringFromTable(@"savePictureFailed", @"XNLocalizable", nil) andRect:CGRectMake((KSCREENSIZE.width - 200)/2, (KSCREENSIZE.height - 90 - 20)/2, 200, 50) andTime:1 inView:self];
            break;
        }
    }
}
